    ...
    <div class="weather-item">
        <div class="header">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
                 width="48" height="48" viewBox="0 0 256 256" xml:space="preserve"
                 @click="favoriteCity()"
                 :class=" this.cityInfo[Object.keys(this.cityInfo)[0]].favorite ? 'active' : ''"
            >
        ...
        <div class="body">
            <WeatherItemInfo  
                :city-info="cityInfo[Object.keys(this.cityInfo)[0]].current"
            />
            <WeatherItemChart v-if="cityInfo[Object.keys(this.cityInfo)[0]].forecast"
                :temp-info="cityInfo[Object.keys(this.cityInfo)[0]].forecast"
            />
        </div>
    </div>
</template>
...
import WeatherItemChart from "@/components/WeatherItemChart";
export default {
    components: {
        WeatherItemInfo, WeatherItemChart
    },
    props: {
        cityInfo: {
            type: Object,
    ...
    methods: {
        onEnter() {
            this.$emit('changeCity', this.cityInfo, this.city)
            this.city = '';
        },
        favoriteCity() {
            this.$emit('favoriteCity', this.cityInfo)
        }
    }
}
</script>
